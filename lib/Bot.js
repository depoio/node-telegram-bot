...
Bot.prototype.sendPhoto = function (options, callback) {
  ...
  if (options.file_id) {
    ...
    var files;
    if (options.files && options.files.stream) {
      files = {
        type: 'photo',
        filename: options.files.filename,
        contentType: options.files.contentType,
    ...
    } else if (options.files && options.files.photo) {
      files = {
        photo: options.files.photo
      }
    } else {
      deferred.reject('Unable to find \'files\' parameter. Please send in files.stream or files.photo');
    }
...
Bot.prototype.sendVoice = function (options, callback) {
  ...
  if (options.file_id) {
    ...
    } else if (options.files && options.files.voice && mime.lookup(options.files.voice) !== 'audio/ogg') {
      return Q.reject(new Error('Invalid file type'))
        .nodeify(callback);
    } else if (options.files && options.files.voice) {
      files = {
        voice: options.files.voice
      }
    } else {
      deferred.reject('Unable to find \'files\' parameter. Please send in files.stream or files.voice');
    }
...
Bot.prototype.sendDocument = function (options, callback) {
  ...
  if (options.file_id) {
    ...
    } else if (options.files && options.files.document) {
      files = {
        document: options.files.document
      }
    } else {
      deferred.reject('Unable to find \'files\' parameter. Please send in files.stream or files.document');
    }
...
Bot.prototype.sendSticker = function (options, callback) {
  ...
  if (options.file_id) {
    ...
    if (options.files && options.files.sticker && mime.lookup(options.files.sticker) !== 'image/webp') {
      return Q.reject(new Error('Invalid file type'))
        .nodeify(callback);
    } else {
      deferred.reject('Unable to find \'files\' parameter. Please send in files.stream or files.sticker');
    }

    if (options.files && options.files.sticker) {
      this._multipart({
        method: 'sendSticker',
        params: {
          chat_id: options.chat_id,
...
Bot.prototype.sendVideo = function (options, callback) {
  ...
  if (options.file_id) {
    ...
    } else if (options.files && options.files.video && mime.lookup(options.files.video) !== 'video/mp4') {
      return Q.reject(new Error('Invalid file type'))
        .nodeify(callback);
    } else if (options.files && options.files.video) {
      files = {
        video: options.files.video
      }
    } else {
      deferred.reject('Unable to find \'files\' parameter. Please send in files.stream or files.video');
    }
