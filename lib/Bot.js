...
Bot.prototype._setWebhook = function (webhook) {
  var self = this;
  var url = this.base_url + this.token + '/setWebhook' + "?" + qs.stringify({url: webhook});
  ...
  request.get({
    ...
    } else if(res.hasOwnProperty('statusCode') && res.statusCode === 401){
      console.log(err);
      console.log("Failed to set webhook with code" + res.statusCode);
    } else {
     console.log('Unknow error, '+err.stack);
    }
  });
}
...
Bot.prototype._poll = function () {
  ...
  request.get({
    ...
    if (!err && res.statusCode === 200) {
      ...
      self.emit('error', 'Unknow error, '+err.stack);
    }
